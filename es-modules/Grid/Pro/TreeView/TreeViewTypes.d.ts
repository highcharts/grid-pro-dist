 ...
 * Tree View options.
 *
 * Turns hierarchical source data into expandable parent and child rows. To
 * build the hierarchy from repeated values in flat data, use
 * {@link RowGroupingOptions | `rowGrouping`} instead.
 */
export interface TreeViewOptions {
    /**
     * Enables tree view processing. It has to be set explicitly, declaring the
     * remaining tree view options alone does not enable the feature.
     *
     * @default false
     */
    enabled?: boolean;
     ...
     * definitions, set `treeView.input.type` explicitly.
     *
 ...
 *
 * Groups flat rows into expandable parent rows by repeated values in the
 * selected source columns. Group rows are generated by the Grid and rendered
 * in the `groupColumnId` column.
 */
export interface RowGroupingOptions {
    /**
     * Enables row grouping. It has to be set explicitly, declaring the
     * remaining row grouping options alone does not enable the feature.
     *
    ...
    enabled?: boolean;
    /**
     * Source column ID or IDs used as row grouping levels, ordered from the
     * top level down.
     *
     * @sample grid-pro/options/row-grouping Row grouping
     */
    ...
    /**
     * Whether the columns listed in `groupBy` are hidden from the rendered
     * table.
     *
     * When `false`, grouped columns stay visible and group rows repeat the
     * group value of their own and their ancestor levels.
     *
     * @default true
     */
 ...
 * TODO: Remove deprecated option before releasing next major
 */
export interface DeprecatedTreeViewOptions extends TreeViewOptions {
    /**
...
/**
 * Number of initially expanded tree levels, or `'all'` for every level.
 */
export type TreeExpandedLevels = (number | 'all');
/**
...
export interface TreeNodeRecord {
    ...
    path?: string;
    /**
     * Group values of the node level and all its ancestor levels, ordered as
     * the configured grouping columns.
     *
     * Set for generated group rows only.
     */
...
export interface TreeInputAdapter {
    buildIndexFromColumns(table: DataTable, input: TreeInputOptions, idColumn?: string): TreeIndexBuildResult;
}
